...
POV1State.transform = sim.map_point_on_lane(lgsvl.Vector(egoX - 4.55 - 6, egoY, egoZ + 3.6))
POV1 = sim.add_agent("Sedan", lgsvl.AgentType.NPC, POV1State)
POV1.on_collision(on_collision)
POVList.append(POV1)

POV2State = lgsvl.AgentState()
POV2State.transform = sim.map_point_on_lane(lgsvl.Vector(POV1State.position.x - 4.6 - 3, POV1State.position.y, POV1State.position.z))
...
for i in range(2,37):
    ...
    POVState.transform = sim.map_point_on_lane(lgsvl.Vector(POVList[i-1].state.position.x - 6, POVList[i-1].state.position.y, POVList[i-1].state.position.z))
