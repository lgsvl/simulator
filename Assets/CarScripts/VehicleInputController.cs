...
public class VehicleInputController : MonoBehaviour, Ros.IRosClient
{
    ...
    void Update()
    {
        ...
        //Debug.Log("monitorVelocity: " + monitorVelocity);
        //Debug.Log("monitorAngVelocity: " + simAngVel);
    }
    ...
    void FixedUpdate()
    {
        if (!keyboard)
        {
            //if (targetLinVel < 0 && !controller.InReverse)
            //{
            ...
            bool linCondition = actualLinVel < targetLinVel;
            //bool angCondition = actualAngVel < targetAngVel;

            var linMag = Mathf.Abs(targetLinVel - actualLinVel);
            if (linCondition/* | controller.InReverse*/)
            {
            ...
            //var angMag = Mathf.Abs(targetAngVel - actualAngVel);
            //if (angCondition)
            //{
            //    controller.steerInput = -Mathf.Clamp(angMag, 0, constSteer);
            //    //controller.steerInput = (controller.InReverse == false ? -1 : 1) * Mathf.Clamp(angMag, 0, constSteer);
            //}
            //else
            //{
            //    controller.steerInput = Mathf.Clamp(angMag, 0, constSteer);
    ...
    public void OnRosConnected()
    {
        ...
        //Bridge.Subscribe(AUTOWARE_CURVEL_CMD_TOPIC, (System.Action<Ros.TwistStamped>)((Ros.TwistStamped msg) =>
        //{
        //    var currentLinear = msg.twist.linear;
        //    var currentAngular = msg.twist.angular;
        //    Debug.Log($"{nameof(currentLinear)} ({currentLinear.x} {currentLinear.y} {currentLinear.z})");
        //    Debug.Log($"{nameof(currentAngular)} ({currentAngular.x} {currentAngular.y} {currentAngular.z})");

        //    if (!keyboard)
        //    {
        //        actualAngVel = (float)currentAngular.z;
        //        actualLinVel = (float)currentLinear.x;
        //    }
        //}));
    }
}
