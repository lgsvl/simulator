...
public class GpsDevice : MonoBehaviour, Ros.IRosClient
{
    ...
    void Update()
    {
        ...
        // float distance = Mathf.Sqrt(pos.x * pos.x + pos.z * pos.z) * Scale;
        // float angularDistance = distance / EarthRadius;
        // float direction = Mathf.Atan2(pos.x, pos.z);
        // north = 0
        // east = 90
        // south = 180
        // west = 270

        //float latitude0 = OriginLatitude * Mathf.Deg2Rad;
        //float longitude0 = OriginLongitude * Mathf.Deg2Rad;

        //float latitude = Mathf.Asin(Mathf.Sin(latitude0) * Mathf.Cos(angularDistance) + Mathf.Cos(latitude0) * Mathf.Sin(angularDistance) * Mathf.Cos(direction));
        //float longitude = longitude0 + Mathf.Atan2(Mathf.Sin(direction) * Mathf.Sin(angularDistance) * Mathf.Cos(latitude0), Mathf.Cos(angularDistance) - Mathf.Sin(OriginLatitude) * Mathf.Sin(latitude));

        //if (longitude < - Mathf.PI)
        //{
        //    longitude += Mathf.PI;
        //}
        //else if (longitude > +Mathf.PI)
        //{
        //    longitude -= Mathf.PI;
        //}
