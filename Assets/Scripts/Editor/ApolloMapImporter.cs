...
namespace Simulator.Editor
{
    ...
    public class ApolloMapImporter
    {
        ...
        void AddLane(string idImported, Lane lane)
        {
            ...
            GameObject mapLaneObj = new GameObject($"MapLane_{IdGenerator.GetIdNumberString(idImported)}");
            ...
            // Imported ID should not be empty, so just overwrite autogenerated value.
            mapLane.id = idImported;

            // TODO set self reverse lanes
            // TODO set direction
            mapLane.leftLineBoundry = Id2LeftLineBoundary[idImported];
            mapLane.rightLineBoundry = Id2RightLineBoundary[idImported];
            ...
            if (junctionId != null) MoveLaneToJunction(new List<string>(){idImported}, junctionId.id.ToString());

            Id2Lane[idImported] = mapLane;
            Id2ApolloLane[idImported] = lane;
        }
        ...
        MapSign CreateStopSign(string idImported, string intersectionId)
        {
            var intersection = Id2MapIntersection[intersectionId];
            var stopLine = Id2StopLine[idImported];
            var stopSignLocation = GetStopSignMeshLocation(stopLine);
            
            var idNumber = IdGenerator.GetIdNumberString(idImported);
            ...
            // Imported ID should not be empty, overwrite autogenerated one.
            mapSign.id = idImported;

            // Create stop sign mesh
            if (IsMeshNeeded)
            {
                CreateStopSignMesh(idNumber, intersection, mapSign);
            }
        ...
        void CreateSignal(Signal signal, string intersectionId)
        {
            ...
            var idImported = signal.id.id.ToString();
            var idNumber = IdGenerator.GetIdNumberString(idImported);
            var intersection = Id2MapIntersection[intersectionId];
            ...
            mapSignal.id = idImported;

            var stopLine = Id2StopLine[mapSignal.id];
            ...
            CreateSignalMesh(idNumber, intersection, mapSignal);
        }
